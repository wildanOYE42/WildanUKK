<?php
class B_pengaduan extends CI_Model {
    ...
    public function getTanggapanByIdJoinAdmin($id){
        ...
        $query = "SELECT * FROM tanggapan INNER JOIN petugas ON tanggapan.id_petugas = petugas.id_petugas WHERE tanggapan.id_pengaduan = $idp ORDER BY tanggapan.id_tanggapan DESC";
        return $this->db->query($query)->result();
    }

    public function add_tanggapan($id, $id_petugas){
    ...
    public function add_laporan(){
        $id = md5($this->input->post('id_pengaduan'));
        $this->db->set('pengaduan',$this->input->post('pengaduan'));
        $this->db->where('md5(id_pengaduan)',$id);
        if($this->db->update('pengaduan')){
            $this->session->set_flashdata('true','laporan berhasil di edit');
            redirect('laporan/add_laporan/' . $id);
        } else {
            $this->session->set_flashdata('false','laporan gagal di edit');
            redirect('laporan/add_laporan/' . $id);
        }
    }
